...
int main() {
    ...
    imgTrainingNumbers = cv::imread("training.png");          // read in training numbers image
    ...
    for (int i = 0; i < ptContours.size(); i++) {                           // for each contour
        if (cv::contourArea(ptContours[i]) > MIN_CONTOUR_AREA) {                // if contour is big enough to consider
            ...
            cv::rectangle(imgTrainingNumbers, boundingRect, cv::Scalar(0, 0, 255), 3);      // draw red rectangle around each contour as we ask user for input
            ...
            cv::namedWindow("imgTrainingNumbers",CV_WINDOW_NORMAL);
            ...
            if (intChar == 27) {        // if esc key was pressed
                ...
                if (intChar >= 97 && intChar <= 122){           //if character is alphabet
                  intChar -= 32;                                //character will capitalized
                }

                std::cout << "intChar = " << intChar << "\n";
                std::cout << "ASCII Char = " << char (intChar) << "\n";
                ...
                }   //end if                                                                            // data types that KNearest.train accepts
            }   // end if
        }   // end if
    }   // end for
