...
std::vector<PossiblePlate> detectPlatesInScene(cv::Mat &imgOriginalScene) {
    ...
    std::cout << "sebelum preprocess" << std::endl;
    ...
    std::cout << "setelah preprocess" << std::endl;
    ...
    std::cout << "masuk findPossibleCharsInScene" << std::endl;
    ...
    std::cout << "masuk findVectorOfVectorsOfMatchingChars" << std::endl;
    ...
    for (unsigned int i = 0; i < vectorOfPossiblePlates.size(); i++) {
        ...
        for (int j = 0; j < 4; j++) {
            cv::line(imgContours, p2fRectPoints[j], p2fRectPoints[(j + 1) % 4], SCALAR_RED, 3);
        }
        cv::imshow("4a", imgContours);
...
std::vector<PossibleChar> findPossibleCharsInScene(cv::Mat &imgThresh) {
    ...
    cv::findContours(imgThreshCopy, contours, CV_RETR_TREE, CV_CHAIN_APPROX_NONE);        // find all contours

    // std::vector<Vec4i> hierarchy;
    //
    // Mat src = cv::imread("plat2.png");
    // Mat src_gray;
    ...
    // int thresh = 170;
    // cv::Canny( src_gray, imgThreshCopy, thresh, thresh*2, 3 );
    // /// Find contours
    // cv::findContours( imgThreshCopy, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );
